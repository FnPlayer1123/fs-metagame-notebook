...
def determineIntervals(winRates, threshold):
        return winRates.apply(lambda x: pandas.Series([x[x >= threshold].first_valid_index(), x[x >= threshold].last_valid_index()], index = ['minv','maxv'])).T

def plotIntervals(intervals, doSort):
...
def main():
        #matplotlib.use('PS')
        filename = sys.argv[1]
        matchups = pandas.read_csv(filename, header=None, index_col = 0)
        ...
        # If the table is set up from 0 to 10, rathen than 0 to 10
        if 1 < matchups.iat[1, 1]:
                matchups = matchups * 0.1

        matchupPayoffs = 2*matchups - 1
        allWinRates = getAllWinRates(matchupPayoffs,10)
        #Plot will output to postscript file
        intervals = determineIntervals(allWinRates, -0.02)
        img = plotIntervals(intervals, True)
